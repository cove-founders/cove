...
2. **Discovered skills** -- 从 `~/.claude/skills`、`~/.agents/skills`、`~/.cursor/skills-cursor`、`~/.opencode/skill`、`~/.officellm/skills` 等约定目录扫描，工作区内还扫描 `.claude/skills` 和 `.agents/skills`（Rust 端 `skill_discovery/mod.rs`）
3. **Built-in skills** -- `src/skills/*/SKILL.md`，通过 Vite `import.meta.glob` 静态加载

同名 Skill 仅保留最高优先级来源。当前优先级分三档：cove(0) > claude(1) > 其他含内置(2)。agents、cursor、opencode、office 及 built-in 同属第三档，无严格排序。
